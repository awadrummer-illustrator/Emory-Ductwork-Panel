...
namespace
{
	...
	bool IsMainDuctLayerName(const std::string& lineLayerName)
	{
		return lineLayerName == "Blue Ductwork" || lineLayerName == "Orange Ductwork";
	}
	...
	bool IsAutoGeneratedPartArt(AIArtHandle art)
	{
		if (!art) {
			return false;
		}
		...
		if (note.find("MD:POINT_ROT=") != std::string::npos ||
			note.find("MDUX_RotationOverride") != std::string::npos ||
			note.find("MDUX_META") != std::string::npos) {
			return true;
		}

		double rotationValue = 0.0;
		return DuctworkMetadata::GetDouble(art, "MDUX_RotationOverride", rotationValue);
	}

	bool GetSinglePointAnchor(AIArtHandle art, DuctworkPoint& outPoint)
	{
		short type = 0;
		if (!art || sAIArt->GetArtType(art, &type) != kNoErr || type != kPathArt) {
			return false;
		}

		std::vector<DuctworkPoint> pathPoints;
		bool closed = false;
		if (!DuctworkGeometry::GetPathPoints(art, pathPoints, closed) || closed || pathPoints.size() != 1) {
			return false;
		}
	...
	bool GetPlacedCenter(AIArtHandle art, DuctworkPoint& outPoint)
	{
		short type = 0;
		if (!art || sAIArt->GetArtType(art, &type) != kNoErr || type != kPlacedArt) {
			return false;
		}
		...
		outPoint.x = (static_cast<double>(bounds.left) + static_cast<double>(bounds.right)) * 0.5;
		outPoint.y = (static_cast<double>(bounds.top) + static_cast<double>(bounds.bottom)) * 0.5;
		return true;
	}

	size_t RemoveAutoGeneratedPartsNearPoint(AILayerHandle layer, const DuctworkPoint& point, double tolerance)
	{
		if (!layer || !sAIArt) {
			return 0;
		}
		...
		for (size_t i = 0; i < art.size(); ++i) {
			if (!IsAutoGeneratedPartArt(art[i])) {
				continue;
			}

			DuctworkPoint artPoint;
			if (!GetSinglePointAnchor(art[i], artPoint) && !GetPlacedCenter(art[i], artPoint)) {
				continue;
			}

			const double dx = artPoint.x - point.x;
			const double dy = artPoint.y - point.y;
			if ((dx * dx + dy * dy) <= toleranceSq) {
				toDelete.push_back(art[i]);
			}
		}

		for (size_t i = 0; i < toDelete.size(); ++i) {
			sAIArt->DisposeArt(toDelete[i]);
		}
	...
	int CountEndpointToSegmentBranchChildren(int pathIndex, const std::vector<DuctworkConnection>& connections)
	{
		...
		for (size_t i = 0; i < connections.size(); ++i) {
			...
			if (conn.endpointA >= 0 && conn.segB >= 0 && conn.b == pathIndex) {
				++count;
				continue;
			}
			if (conn.endpointB >= 0 && conn.segA >= 0 && conn.a == pathIndex) {
				++count;
				continue;
			}
		}
	...
	bool PathHasSegmentIntersectionConnection(int pathIndex, const std::vector<DuctworkConnection>& connections)
	{
		for (size_t i = 0; i < connections.size(); ++i) {
			const DuctworkConnection& conn = connections[i];
			if (conn.type == kConnectionSegmentIntersection &&
				(conn.a == pathIndex || conn.b == pathIndex)) {
				return true;
			}
		}
		return false;
	...
	bool HasInteriorSegmentIntersectionGeometry(size_t pathIndex,
		const DuctworkPath& path,
		const std::vector<DuctworkPath>& paths,
		double clearance)
	{
	...
	bool CanPlaceDistributionTrunkTerminalRegisters(const DuctworkPath& path,
		int pathIndex,
		const std::vector<DuctworkConnection>& connections,
		const std::vector<DuctworkPoint>& unitAnchors,
		double unitTolerance)
	{
		if (path.closed || path.points.size() < 3) {
			return false;
		}

		const int endIndex = static_cast<int>(path.points.size() - 1);
		if (EndpointIsConnected(pathIndex, 0, connections) ||
			EndpointIsConnected(pathIndex, endIndex, connections)) {
			return false;
		}

		if (IsPointNear(path.points.front(), unitAnchors, unitTolerance) ||
			IsPointNear(path.points.back(), unitAnchors, unitTolerance)) {
			return false;
		}
	...
	bool ShouldForceSegmentIntersectionTerminalRegisters(const std::string& registerLayerName,
		int pathIndex,
		const DuctworkPath& path,
		const std::vector<DuctworkPath>& paths,
		const std::vector<DuctworkConnection>& connections,
		const std::vector<DuctworkPoint>& unitAnchors)
	{
		if (registerLayerName == "Thermostats" || path.closed || path.points.size() < 2) {
			return false;
		}
		if (IsPointNear(path.points.front(), unitAnchors, 10.0) ||
			IsPointNear(path.points.back(), unitAnchors, 10.0)) {
			return false;
		}
		return PathHasSegmentIntersectionConnection(pathIndex, connections) ||
			HasInteriorSegmentIntersectionGeometry(static_cast<size_t>(pathIndex), path, paths, 10.0);
	}
	...
	void CollectDocumentEndpointsByLayer(const std::string& layerName,
		std::vector<DuctworkPoint>& outPoints,
		std::vector<double>& outRotations)
	{
		outPoints.clear();
		outRotations.clear();
		if (layerName.empty() || !sAIArt) {
			return;
		}
		...
		for (size_t i = 0; i < art.size(); ++i) {
			DuctworkPath path;
			bool closed = false;
			if (!DuctworkGeometry::GetPathPoints(art[i], path.points, closed)) {
				continue;
			}
			path.art = art[i];
			path.closed = closed;
	...
	void CollectDocumentPathsByLayer(const std::string& layerName,
		std::vector<DuctworkPath>& outPaths)
	{
		outPaths.clear();
		if (layerName.empty() || !sAIArt) {
			return;
	...
	void EnsureEndpointsAvailable(const std::vector<DuctworkPath>& paths,
		const std::string& layerName,
		std::vector<DuctworkPoint>& outPoints,
		std::vector<double>& outRotations,
		bool& supplementedFromDocument)
	{
		supplementedFromDocument = false;
		CollectEndpointsByLayer(paths, layerName, outPoints, outRotations);
		if (!outPoints.empty()) {
			return;
		}
		CollectDocumentEndpointsByLayer(layerName, outPoints, outRotations);
		supplementedFromDocument = !outPoints.empty();
	}

	double FindNearestEndpointDistance(const std::vector<DuctworkPoint>& a,
		const std::vector<DuctworkPoint>& b)
	{
		if (a.empty() || b.empty()) {
			return -1.0;
		}
		...
		for (size_t i = 0; i < a.size(); ++i) {
			for (size_t j = 0; j < b.size(); ++j) {
				const double dist2 = DuctworkMath::Dist2(a[i], b[j]);
				if (dist2 < bestDist2) {
					bestDist2 = dist2;
				}
			}
		}
		return bestDist2 < (std::numeric_limits<double>::max)() ? std::sqrt(bestDist2) : -1.0;
	}
	...
	bool placedApiGraphics)
{
	...
	bool supplementedGreen = false;
	bool supplementedLightGreen = false;
	bool supplementedBlue = false;
	bool supplementedOrange = false;
	bool supplementedLightOrange = false;
	bool supplementedThermostat = false;

	EnsureEndpointsAvailable(paths, "Green Ductwork", greenPts, greenRot, supplementedGreen);
	EnsureEndpointsAvailable(paths, "Light Green Ductwork", lightGreenPts, lightGreenRot, supplementedLightGreen);
	EnsureEndpointsAvailable(paths, "Blue Ductwork", bluePts, blueRot, supplementedBlue);
	EnsureEndpointsAvailable(paths, "Orange Ductwork", orangePts, orangeRot, supplementedOrange);
	EnsureEndpointsAvailable(paths, "Light Orange Ductwork", lightOrangePts, lightOrangeRot, supplementedLightOrange);
	EnsureEndpointsAvailable(paths, "Thermostat Lines", tstatPts, tstatRot, supplementedThermostat);

	{
		std::ostringstream logStream;
		logStream << "Units: endpointCounts"
			<< " green=" << greenPts.size()
			<< " lightGreen=" << lightGreenPts.size()
			<< " blue=" << bluePts.size()
			<< " orange=" << orangePts.size()
			<< " lightOrange=" << lightOrangePts.size()
			<< " thermostat=" << tstatPts.size()
			<< " supplementedGreen=" << (supplementedGreen ? 1 : 0)
			<< " supplementedLightGreen=" << (supplementedLightGreen ? 1 : 0)
			<< " supplementedBlue=" << (supplementedBlue ? 1 : 0)
			<< " supplementedOrange=" << (supplementedOrange ? 1 : 0)
			<< " supplementedLightOrange=" << (supplementedLightOrange ? 1 : 0)
			<< " supplementedThermostat=" << (supplementedThermostat ? 1 : 0)
			<< " nearestThermostatBlue=" << FindNearestEndpointDistance(tstatPts, bluePts);
	...
	bool placedApiGraphics)
{
	...
	if (grouped.find("Thermostats") == grouped.end()) {
		std::vector<DuctworkPath> documentThermostatPaths;
		CollectDocumentPathsByLayer("Thermostat Lines", documentThermostatPaths);
		if (!documentThermostatPaths.empty()) {
			std::vector<int>& thermostatIndices = grouped["Thermostats"];
			const size_t firstIndex = partPaths.size();
			partPaths.insert(partPaths.end(), documentThermostatPaths.begin(), documentThermostatPaths.end());
			for (size_t i = 0; i < documentThermostatPaths.size(); ++i) {
				thermostatIndices.push_back(static_cast<int>(firstIndex + i));
			}
			DuctworkLog::Write("Parts: supplemented Thermostats paths from document count=" +
				std::to_string(documentThermostatPaths.size()));
		}
	}
	...
	std::vector<DuctworkPoint> runtimeIgnoreAnchors;
	{
		...
		for (size_t pathIndex = 0; pathIndex < partPaths.size(); ++pathIndex) {
			const DuctworkPath& path = partPaths[pathIndex];
			if (!IsMainDuctLayerName(path.layerName) || path.closed || path.points.size() < 2) {
				continue;
			}
	...
	CollectAnchorsFromLayerName("Units", unitTerminalAnchors);
	CollectGraphicsFromLayerName("Units", unitTerminalAnchors);
	...
	for (std::map<std::string, std::vector<int> >::iterator it = grouped.begin();
		...
		size_t removedObsoleteParts = 0;
		for (size_t p = 0; p < it->second.size(); ++p) {
			int pathIndex = it->second[p];
			const DuctworkPath& path = partPaths[static_cast<size_t>(pathIndex)];
			if (path.points.size() < 2 || path.closed) {
				continue;
			}

			const int branchChildCount = CountEndpointToSegmentBranchChildren(pathIndex, connections);
			const bool distributionTrunk = (registerLayerName != "Thermostats") && branchChildCount >= 2;
			const bool allowTerminalRegistersOnTrunk = distributionTrunk &&
				CanPlaceDistributionTrunkTerminalRegisters(path, pathIndex, connections, skipAnchors, 10.0);
			const bool forceSegmentIntersectionTerminals = ShouldForceSegmentIntersectionTerminalRegisters(
				registerLayerName,
				pathIndex,
				path,
				partPaths,
		...
		if (removedObsoleteParts > 0) {
			DuctworkLog::Write("Parts: removed obsolete autogenerated parts layer=" + registerLayerName +
				" count=" + std::to_string(removedObsoleteParts));
		}
		if (registerLayerName != "Thermostats") {
			size_t removedIgnoredParts = 0;
			for (size_t i = 0; i < runtimeIgnoreAnchors.size(); ++i) {
				removedIgnoredParts += RemoveAutoGeneratedPartsNearPoint(registerLayer,
					runtimeIgnoreAnchors[i],
					anchorTolerance + 2.0);
			}
			if (removedIgnoredParts > 0) {
				DuctworkLog::Write("Parts: removed ignored-end autogenerated parts layer=" + registerLayerName +
					" count=" + std::to_string(removedIgnoredParts));
			}
		}
		...
		for (size_t p = 0; p < it->second.size(); ++p) {
			const DuctworkPath& path = partPaths[static_cast<size_t>(it->second[p])];
			if (path.closed || path.points.size() < 2) {
				continue;
			}
			endpointCount += 2;
		...
		for (size_t p = 0; p < it->second.size(); ++p) {
			...
			const bool forceSegmentIntersectionTerminals = ShouldForceSegmentIntersectionTerminalRegisters(
				...
				connections,
				unitTerminalAnchors);
			if (distributionTrunk) {
				std::ostringstream logStream;
				logStream << "Parts: distribution trunk layer=" << registerLayerName
					<< " pathIndex=" << pathIndex
					<< " branchChildren=" << branchChildCount
					<< " allowTerminalRegisters=" << (allowTerminalRegistersOnTrunk ? 1 : 0);
				DuctworkLog::Write(logStream.str());
			}
			if (forceSegmentIntersectionTerminals) {
				std::ostringstream logStream;
				logStream << "Parts: segment-intersection terminals layer=" << registerLayerName
					<< " pathIndex=" << pathIndex
					<< " allowTerminalRegisters=1";
				DuctworkLog::Write(logStream.str());
			}
			...
			for (size_t e = 0; e < 2; ++e) {
				...
				double angle = 0.0;
				if (gRotateRegisters) {
					angle = gHasRotationOverride ? gRotationOverride : ComputeEndpointAngle(path, idx);
				}
